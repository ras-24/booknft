...
import "https://raw.githubusercontent.com/OpenZeppelin/openzeppelin-contracts/v4.3.2/contracts/token/ERC721/ERC721.sol";
...
contract BookNFT is ERC721, Ownable {
    ...
    event NFTDonated(uint256 indexed id, address indexed from, address indexed to);
    event ResalePriceSet(uint256 indexed bookId, address indexed owner, uint256 price);

    constructor() ERC721("GathbookNFT", "GBOOK") {}
    ...
    function createBook(
        ...
        require(initialRoyalty <= 100, "Initial royalty must be between 0 and 100");
        require(resaleRoyalty <= 100, "Resale royalty must be between 0 and 100");
    ...
    function buyNFT(uint256 bookId) external payable {
        ...
        if (bookOwners[bookId].length == 0 || bookOwners[bookId][bookOwners[bookId].length - 1] != msg.sender) {
    ...
    function buyResaleNFT(uint256 bookId, address owner) external payable {
        ...
        require(msg.value >= resalePrice, "Insufficient funds to buy the resale NFT");

        _transfer(owner, msg.sender, bookId);
        books[bookId].ownedNFTs[owner]--;
        books[bookId].ownedNFTs[msg.sender]++;

        uint256 resaleRoyaltyAmount = (resalePrice * books[bookId].resaleRoyalty) / 100; 
    ...
    function getResalePrice(uint256 bookId, address owner) external view returns (uint256) {
        return books[bookId].resalePrices[owner];
    }

    function getNFTPurchaseInfo(uint256 bookId) external view returns (uint256 purchased, uint256 totalSupply) {
        Book storage book = books[bookId];
    ...
    function getTransactionCount(uint256 bookId) external view returns (uint256) {
